...
if st.button("Run Simulation"):
    ...
    sim_proba = float(risk_model.predict_proba(sim_data)[:, 1][0])  # ✅ FIXED
    sim_delay = float(delay_model.predict(sim_data)[0])             # ✅ FIXED
...
if st.sidebar.button("Run Scenario Simulation"):
    ...
    for label, vals in scenarios.items():
        ...
        risk_proba = float(risk_model.predict_proba(df)[:, 1][0])   # ✅ FIXED
        delay_est = float(delay_model.predict(df)[0])               # ✅ FIXED
